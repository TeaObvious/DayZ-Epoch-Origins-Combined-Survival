...
*/
private ["_isOk","_i","_objName","_started","_finished","_animState","_isMedic","_proceed","_itemOut","_countOut","_tree","_findNearestTree","_index","_invResult","_text","_plantInfo","_plantType","_plantGrownUp","_plantObjects","_plantOutput"];
...
_findNearestTree = [];
{
	if (alive _x) then {
		...
		if (_plantType != "undef" && _plantGrownUp) exitWith {
			...
			_itemOut = (_plantObjects select 0) select 0;
			_countOut = (_plantObjects select 0) select 1; 
		};
	};
